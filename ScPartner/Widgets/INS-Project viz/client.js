function projectVizClient($scope) {
    ...
    c.progressBarState = function (row) {
        var incident = c.data.incidents[row.$parent.$index];
        var cl = {
            first: row.$first,
            active: row.state.value.value == incident.state.value,
            next: row.state.value.value == (incident.state.value + 1),
            visited: row.state.value.value < incident.state.value,
            previous: row.state.value.value == (incident.state.value - 1)
        };
        cl.fa = cl.active;
